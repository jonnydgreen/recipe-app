        ...
        <Form method="post" className="text-gray-900">
          <div className="flex flex-col">
            <div className="mr-4 w-full py-2">
              <b>Name:</b>
              <TextField name="name" defaultValue="Fave recipe" />
            </div>
            <div className="mr-4 w-full py-2">
              <b>Ingredients:</b>
              <div>
                {[...Array(ingredients)].map((i, index) => (
                  <div key={`${index}`} className="flex py-2">
                    <div key={`ingredient.${index}.name`} className="px-2">
                      <TextField name={`ingredient.${index}.name`} defaultValue="Sugar" />
                    </div>
                    <div key={`ingredient.${index}.value`} className="px-2">
                      <TextField name={`ingredient.${index}.value`} defaultValue="1" />
                    </div>
                    <div key={`ingredient.${index}.unit`} className="px-2">
                      {/* TODO: select box */}
                      <TextField name={`ingredient.${index}.unit`} defaultValue="G" />
                    </div>
                    <Button
                ...
                {[...Array(method)].map((i, index) => (
                  <div key={`${index}`} className="flex py-2">
                    <div key={`method.${index}.instructions`} className="px-2">
                      <TextField name={`method.${index}.instructions`} defaultValue="Add it in" />
                    </div>
                    <Button
            ...
            <Button className="bg-cyan-700 hover:bg-cyan-600 py-2" type="submit" onClick={onClickHandler}>
              Confirm
            </Button>
          </div>
        </Form>
